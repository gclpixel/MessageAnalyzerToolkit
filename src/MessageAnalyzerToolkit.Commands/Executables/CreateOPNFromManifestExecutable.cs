...
namespace MessageAnalyzerToolkit.Executables
{
    internal class CreateOPNFromManifestExecutable : IExecutable
    {
        ...
        private void WriteKeywords(StringBuilder sb, XElement provider)
        {
            ...
            sb.AppendLine();
            List<Tuple<string, long>> keywords = new List<Tuple<string, long>>();
            foreach (var keyword in provider.Element(GetXName("keywords")).Elements(GetXName("keyword")))
            {
                ...
                keywords.Add(new Tuple<string, long>(keywordName, mask));
            }

            Dictionary<string, string> keywordExt = new Dictionary<string, string>();
            long highestKeyWord = 0x1000000000000;
            foreach (var keyword in keywords.OrderByDescending(p => p.Item2))
            {
                long padding = (long)(Math.Round(Math.Log(highestKeyWord, 2)) - Math.Round(Math.Log(keyword.Item2, 2)));
                if (padding > 1)
                {
                    keywordExt.Add(keyword.Item1, ", LeadPadding = " + (padding - 1));
                }
                highestKeyWord = keyword.Item2;
            }

            foreach (var keyword in keywords.OrderByDescending(p => p.Item2))
            {
                string ext;
                if (!keywordExt.TryGetValue(keyword.Item1, out ext))
                {
                    ext = string.Empty;
                }

                sb.AppendFormat("\tbool {0} with Standard.BinaryEncoding{{Width = 1{1}}};", keyword.Item1, ext);
                sb.AppendLine();
                sb.AppendLine();
            }
        ...
        private void WriteETWActor(StringBuilder sb, XElement provider, Dictionary<Tuple<int, int, string>, XElement> events)
        {
            ...
            foreach (var etwEventGroup in events.GroupBy(p => p.Key.Item2))
            {
                ...
                foreach (var etwEvent in etwEventGroup)
                {
                    ...
                    sb.AppendLine();
                    sb.AppendLine("\t\t\t\t\t\t\t{");
                    sb.AppendLine("\t\t\t\t\t\t\t\tdecodedMsg.EtwKeywords = DecodeEtwKeywords<Keywords>(m);");
        ...
        private static void WriteTemplates(StringBuilder sb)
        {
            //foreach (var template in provider.Element(GetXName("templates")).Elements(GetXName("template")))
            //{
            //    sb.AppendFormat("message {0}", template.Attribute("tid").Value);
            //    sb.AppendLine();
            //    sb.AppendLine("{");

            //    foreach (var data in template.Elements(GetXName("data")))
            //    {
            //        sb.AppendFormat("\t{0} {1};", ConvertETWType(data.Attribute("inType").Value), ConvertToPascalCase(data.Attribute("name").Value));
            //        sb.AppendLine();
            //        sb.AppendLine();
            //    }
